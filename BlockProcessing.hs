...
import StringUtils (countOccurrences, isOnlyCharOrSpace, repeatTagPair)
...
markHorizontalRules list = case list of
  ...
  (x, y) : xs ->
    ...
    where
      isHRule = isOnlyCharOrSpace '-' x && countOccurrences x '-' > 2 || isOnlyCharOrSpace '*' x && countOccurrences x '*' > 2
